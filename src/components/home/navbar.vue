<template>
  <nav class="d-flex pt-4 pl-6 pr-6 pb-2 justify-space-between align-center">
    <div class="font__sm font__white font__bold d-flex align-center">
      <button v-if="$route.meta.child" class="mr-2" @click="goBack">
        <v-icon color="#ffffff">mdi-arrow-left</v-icon>
      </button>
      <span> {{ appName }}</span>
    </div>
    <div class="d-flex align-center">
      <router-link to="/" class="mr-12 home__link font__x__sm">
        Home
      </router-link>
      <router-link to="/about" class="mr-12 home__link font__x__sm">
        About Us
      </router-link>
      <router-link to="/dashboard" class="mr-12">
        <cbutton :text="'Dashboard'" :pl="''" :pr="''" :mt="''" />
      </router-link>
    </div>
  </nav>
</template>
